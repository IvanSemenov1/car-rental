        ...
        <div className='group relative'>
            <label htmlFor="image">
                <img src={image ? URL.createObjectURL(image) : user?.image ||
                    "https://images.unsplash.com/photo-1633332755192-727a05c4013d?q=80&w=300"} alt="" className='h-9 
      ...
      <div className='w-full'>
        {ownerMenuLinks.map((link, index) => (
            ...
            py-3 pl-4 first:mt-6 ${link.path === location.pathname ? 'bg-primary/10 text-primary' : 
                ...
                <div className={`${link.path === location.pathname && 'bg-primary'} w-1.5 h-8 rounded-l 
                right-0 absolute`}></div>
            </NavLink>
        ))}
      </div>
    </div>
